...
```{r}
wineData<-loadDataSet("wine.data.csv",header=TRUE,sep=",")
wineData
...
wineData<-discretize(wineData,5,"EF",14)
...
The function normalize is used to normalize the data or an attribute. When applied to a DataSet, columns parameter indicate to which columns the normalization have to be applied. In this case I will apply it to the following columns: 3,4,5,7,8,10,11,12,13. 


```{r, fig.width=10, fig.height=6, out.width="100%"}

wineDataNormalized<-normalize(wineData,c(3,4,5,7,8,10,11,12,13))
wineDataNormalized
```
...
To standardize the data or an attribute the function standardize can be used. When applied to a DataSet, columns parameter indicate to which columns the normalization have to be applied. In this case I will apply it to the following columns: 3,4,5,7,8,10,11,12,13. 


```{r, fig.width=10, fig.height=6, out.width="100%"}

wineDataStandardized<-standardize(wineData,c(3,4,5,7,8,10,11,12,13))
...
```
The entropy of single attributes can also be computed. If the attribute is categorical the result will be Na.
